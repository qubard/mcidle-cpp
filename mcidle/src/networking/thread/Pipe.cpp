...
void Pipe::Start()
{
    for(;;)
    {
        ...
        while (!m_Queue.Empty())
        {
            ...
            {
                // Send each packet
                if (pkt->Id() != 0x22 && pkt->Id() != 0x12 && pkt->Id() != 0xC && pkt->Id() != 0x3e && pkt->Id() != 0x04 && pkt->Id() != 0x26 && pkt->Id() != 0x36 && pkt->Id() != 0x27 && pkt->Id() != 0x4C && pkt->Id() != 0x05 && pkt->Id() != 0x06 && pkt->Id() != 0x07 && pkt->Id() != 0x0B)
                {
