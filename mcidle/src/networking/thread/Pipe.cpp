...
Pipe::Pipe(u32 rate, u32 maxSize) : m_RateMS(rate), m_MaxSize(maxSize) {}

Pipe::Pipe(u32 rate, u32 maxSize, std::shared_ptr<Connection> sink) :
	m_RateMS(rate), m_MaxSize(maxSize), m_Sink(sink)
{
}

void Pipe::SetSink(
	std::shared_ptr<Connection> sink, std::shared_ptr<Pipe> sinkPipe)
{
