...
ByteBuffer::ByteBuffer(std::vector<u8> & data) :
	m_BigEndian(true), m_ReadOffset(0), m_WriteOffset(0)
{
...
ByteBuffer::ByteBuffer(std::size_t capacity, bool bigEndian = true) :
	m_BigEndian(bigEndian), m_ReadOffset(0), m_WriteOffset(0)
{
...
bool ByteBuffer::Avail() const { return m_ReadOffset < m_Data.size(); }

void ByteBuffer::SeekRead(std::size_t offset) { m_ReadOffset = offset; }

void ByteBuffer::SeekWrite(std::size_t offset) { m_WriteOffset = offset; }
...
std::size_t ByteBuffer::ReadOffset() const { return m_ReadOffset; }
...
void ByteBuffer::Reserve(std::size_t capacity) { m_Data.reserve(capacity); }
...
u8 * ByteBuffer::Front() { return m_Data.data(); }

u8 * ByteBuffer::Back() { return m_Data.data() + m_Data.size(); }
...
std::size_t ByteBuffer::WriteSize() const { return m_WriteOffset; }
...
u8 & ByteBuffer::operator[](std::size_t idx) { return m_Data[idx]; }
...
template <> VarInt ByteBuffer::Read()
{
	VarInt res;
	*this >> res;
	return res;
...
template <> void ByteBuffer::Write(const VarInt val) { *this << val; }
