...
void GameState::SetPosition(
	double x, double y, double z, bool relX, bool relY, bool relZ)
{
	boost::lock_guard<boost::mutex> guard(m_Mutex);
	...
	else
	{
		...
		for (s32 y = 0; y < game::SECTION_SIZE; y++)
		{
			if (chunk->PrimaryBitMask & (1 << y))
			{
				...
				// Make sure to update the mask to include the new or updated
				// section
...
void GameState::Lock() { m_Mutex.lock(); }

void GameState::Unlock() { m_Mutex.unlock(); }

void GameState::UpdateEntityPosition(
	s32 id, double x, double y, double z, double vx, double vy, double vz)
{
	boost::lock_guard<boost::mutex> guard(m_Mutex);
