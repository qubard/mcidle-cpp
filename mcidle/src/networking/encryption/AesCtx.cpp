...
bool AesCtx::Initialize(std::string & publicKey, std::string & verifyToken)
{
	...
	RSA * rsa =
		d2i_RSA_PUBKEY(NULL, (const u8 **)&pubKeyAddr, publicKey.size());
	...
	RSA_public_encrypt(
		sharedSecret.size(), (u8 *)&sharedSecret[0], (u8 *)&encSecret[0], rsa,
		RSA_PKCS1_PADDING);
	// Encrypt the verify token with public key
	RSA_public_encrypt(
		verifyToken.size(), (const u8 *)verifyToken.c_str(),
...
bool AesCtx::InitializeCtx(std::string & sharedSecret)
{
	...
	if (!(EVP_EncryptInit_ex(
			m_EncryptCtx, EVP_aes_128_cfb8(), nullptr,
	...
	if (!(EVP_DecryptInit_ex(
			m_DecryptCtx, EVP_aes_128_cfb8(), nullptr,
			(const u8 *)sharedSecret.c_str(),
			(const u8 *)sharedSecret.c_str())))
		return false;
...
std::string & AesCtx::EncSecret() { return m_EncSecret; }

std::string & AesCtx::Secret() { return m_Secret; }

std::string & AesCtx::EncToken() { return m_EncToken; }
