...
ChunkData::ChunkData(
	s32 chunkX, s32 chunkZ, bool groundUp, s32 primaryBitMask) :
	Packet(),
	m_ChunkX(chunkX),
	m_ChunkZ(chunkZ),
	m_GroundUp(groundUp),
...
ChunkData::Response(Protocol & protocol, s32 compression)
{
	// Generate a serverbound keep alive using our id
	return nullptr;
}

s32 ChunkData::ChunkX() const { return m_ChunkX; }

s32 ChunkData::ChunkZ() const { return m_ChunkZ; }

inline void
ChunkData::WriteSection(ByteBuffer & buf, s32 section, u8 bitsPerBlock)
{
	buf << bitsPerBlock;
...
inline void
ChunkData::ReadSection(ByteBuffer & buf, int ChunkX, int ChunkZ, int section)
{
	u8 bits_per_block;
	buf >> bits_per_block;
