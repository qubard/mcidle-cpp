...
TagCompound::TagCompound() { m_Tags = std::vector<std::shared_ptr<Tag>>(); }

void TagCompound::Push(std::shared_ptr<Tag> tag) { m_Tags.push_back(tag); }

void TagCompound::Serialize(ByteBuffer & buf) { buf << *this; }

std::vector<std::shared_ptr<Tag>> & TagCompound::Tags() { return m_Tags; }

bool TagCompound::Empty() const { return m_Tags.size() == 0; }

std::size_t TagCompound::Size() const { return m_Tags.size(); }
