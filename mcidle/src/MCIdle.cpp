...
MCIdle::MCIdle(
	bool online, std::string serverIP, std::string port,
	std::shared_ptr<Protocol> protocolCB, std::shared_ptr<Protocol> protocolSB,
	util::Yggdrasil yggdrasil) :
	m_Online(online),
	m_ServerIP(serverIP),
	m_Port(port),
...
std::shared_ptr<game::GameState> MCIdle::GameState() { return m_State; }
...
bool MCIdle::Start()
{
	...
	boost::thread([&]() {
		for (;;)
		{
			...
			auto mc_conn = std::make_shared<mcidle::Connection>(
				conn, protocol, state, 4096);
			...
			if (m_Online)
			{
				...
				mc_conn->SendPacket(
					mcidle::packet::clientbound::EncryptionRequest(
						"", pubKey, CToken));
			...
			// This is buggy..need to read compression then set the protocol
			// state.. or it doesn't matter
			...
			for (auto & slot : inv)
			{
				mcidle::packet::clientbound::SetSlot p(
					0, slot.first, slot.second);
				printf(
					"About to send set slot for slot num :%d item id short "
					"%d\n",
					slot.first, slot.second.ItemIDShort);
				mc_conn->SendPacket(p);
				printf(
					"Sent set slot for slot num :%d item id short %d\n",
					slot.first, slot.second.ItemIDShort);
			}
			...
			// If we hang here then the player joins properly with chunks
			// received The problem is the socket closes before we can do that
