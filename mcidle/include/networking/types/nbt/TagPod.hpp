...
template <typename T> class TagPod : public Tag
{
  public:
	template <
		class Q = T,
		typename std::enable_if<std::is_same<Q, s8>::value>::type * = nullptr>
	...
	template <
		class Q = T,
		typename std::enable_if<std::is_same<Q, s32>::value>::type * = nullptr>
	void WriteType(ByteBuffer & buf)
	{
		buf << TAG_INT;
	}
	...
	template <
		class Q = T,
		typename std::enable_if<std::is_same<Q, s16>::value>::type * = nullptr>
	void WriteType(ByteBuffer & buf)
	{
		buf << TAG_SHORT;
	}
	...
	template <
		class Q = T, typename std::enable_if<
						 std::is_same<Q, NBTString>::value>::type * = nullptr>
	void WriteType(ByteBuffer & buf)
	{
		buf << TAG_STRING;
	}
	...
	template <
		class Q = T, typename std::enable_if<
						 std::is_same<Q, double>::value>::type * = nullptr>
	void WriteType(ByteBuffer & buf)
	{
		buf << TAG_DOUBLE;
	}
	...
	template <
		class Q = T, typename std::enable_if<
						 std::is_same<Q, float>::value>::type * = nullptr>
	void WriteType(ByteBuffer & buf)
	{
		buf << TAG_FLOAT;
	}
